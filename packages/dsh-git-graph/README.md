...
- The host half's `/git/*` only accepts paths of registered workspaces (realpath check) and loopback clients (loopback socket + loopback Host, the family loopback fence); the browser cannot run git against arbitrary directories, and a LAN-exposed dsh web answers non-loopback clients with 403.
