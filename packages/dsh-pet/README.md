...
- `pet.json`: `id`, `displayName`, `description`, `spritesheetPath: "spritesheet.webp"`, and fixed frame counts `[6, 8, 8, 4, 5, 8, 6, 6, 6]`.
- `spritesheet.webp`: transparent WebP, `1536×1872`, 8 columns × 9 rows, `192×208` cells; rows are `idle`, `running-right`, `running-left`, `waving`, `jumping`, `failed`, `waiting`, `running`, `review`.
